...
{
    _cyhal_tcpwm_register_callback(&(obj->resource), (cy_israddress) callback, callback_arg);
}

#define cyhal_pwm_register_callback(obj, callback, callback_arg) _cyhal_pwm_register_callback(obj, callback, callback_arg)
...
__STATIC_INLINE void _cyhal_pwm_enable_event(cyhal_pwm_t *obj, cyhal_pwm_event_t event, uint8_t intr_priority, bool enable)
{
    ...
    _cyhal_tcpwm_enable_event(obj->base, &(obj->resource), converted, intr_priority, enable);
}

#define cyhal_pwm_enable_event(obj, event, intr_priority, enable) _cyhal_pwm_enable_event(obj, event, intr_priority, enable)
